...
|---|---|---|
| [LLM Serving Platform](https://github.com/xiyiji/llm-serving-platform) | A self-hosted serving layer for LLM inference: OpenAI-compatible gateway with SSE streaming, latency-aware routing, dynamic micro-batching, prefix caching (**94.7%** hit rate under repeated-prompt load), warm-pool cold-start management, canary releases with **auto-rollback**, and a Next.js ops console. Pairs with [InferenceGateway](https://github.com/xiyiji/InferenceGateway) (Ray Serve + vLLM) as the engine layer. `27 tests` · CI · Docker/K8s/Terraform | Python · FastAPI · Next.js · TypeScript · Prometheus |
