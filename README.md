...
- 🤝 **Human-gated multi-agent execution** — five specialist agents behind one control-plane orchestrator, with a deterministic risk floor, approval gates, bounded retry/rework, SQL-backed task and event records, and a no-key demo; **46 offline tests** (Atlas)
...
| [Atlas](https://github.com/xiyiji/atlas-llm-execution-agent) ([live demo](https://xiyiji.github.io/projects/atlas-demo.html)) | A demo-first control plane for multi-agent execution. Planner, Safety, Coder, Browser and Verifier agents sit behind **one orchestrator that owns lifecycle policy and never calls a model itself**. Model risk is merged with a deterministic floor; high-risk work **waits for a human decision before execution**. The no-key demo covers planning, isolated local code execution, verification, one rework round, SQL-backed task/event records, restart recovery and an SSE mission-control UI. `46 offline tests` · ruff · pip-audit · CI. PostgreSQL, Redis, Celery and Docker Compose are implemented as a production-oriented foundation, with distributed integration hardening still on the roadmap. | Python · FastAPI · SQLAlchemy · Celery · Redis · SSE |
